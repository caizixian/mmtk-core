...
## Progress
- Starting count: 722 | Current: 608 (before this step) | Δ: -114
- Phase: 2
...
1. 🔴 HIGH: `src/policy/marksweepspace/native_ms/block.rs:104-120` — Check if `load` calls on tables can be abstracted with a safe wrapper.
2. 🟡 MED: `src/util/metadata/side_metadata/helpers.rs:548-557` — Remove unnecessary unsafe blocks (identified by compiler warnings).
3. 🟡 MED: `src/util/metadata/side_metadata/ranges.rs:177` — Remove unnecessary unsafe block.
