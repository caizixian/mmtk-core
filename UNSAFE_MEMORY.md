...
## Progress
- Starting count: 715 | Current: 477 | Δ: -238
- Completed subsystems: policy/immix/line.rs (refactored to use safe atomic load/store, eliminating all unsafe blocks), util/int_array_freelist.rs (refactored to use `Arc<Mutex<Vec<i32>>>` instead of raw pointers and `NonNull`, eliminating `unsafe impl Send/Sync` and raw pointer dereferences), util/metadata/vo_bit, util/alloc/free_list_allocator.rs, policy/marksweepspace (and native_ms/block.rs safe load_atomic), util/heap/layout/map32.rs (removed unnecessary unsafe block, removed `new_zeroed_vec` usage), util/heap/layout, util/copy, util/metadata/side_metadata, util/heap/gc_trigger.rs, util/metadata/header_metadata.rs, util/heap/blockpageresource.rs (eliminated push_relaxed and replace unsafe methods in BlockQueue by using Mutex for local queues, removed UnsafeCell from data storage as concurrent writes are no longer performed without &mut self), scheduler/gc_work.rs, util/metadata/vo_bit (made unset_vo_bit_unsafe and is_vo_addr safe with Relaxed atomics, removed unnecessary unsafe block), util/linear_scan, vm/tests/mock_tests/mock_test_slots.rs, util/heap/freelistpageresource.rs, util/rust_util (replaced libc::getpid with std::process::id to remove unsafe block), policy/sft_map (SFTMap update/eager_initialize take reference, remove unsafe in implementations), policy/marksweepspace/malloc_ms/global.rs (unnecessary SFT_MAP unsafe, safe page marks, removed dead bulk sweep code), policy/lockfreeimmortalspace.rs (unnecessary eager_initialize unsafe), mmtk.rs (removed unnecessary unsafe cast for GCTrigger mutation, removed unnecessary unsafe impl Send), policy/marksweepspace/malloc_ms/metadata.rs (safe page marks, removed unused load128), util/metadata/side_metadata/global.rs (combined unsafe blocks in bcopy_metadata_contiguous), util/heap/monotonepageresource.rs (made reset/release_pages safe), policy/copyspace.rs (removed unnecessary unsafe), plan/barriers.rs (safe load_atomic), util/test_util/fixtures.rs (refactored MMTKFixture to use &mut MMTK, removed unsafe impl Send for MutatorFixture, removed unsound blanket unsafe impl Sync for Fixture), docs/dummyvm/src/lib.rs (replaced from_raw_address_unchecked with safe alternative), vm/slot.rs (refactored SimpleSlot to use Address, eliminating unsafe impl Send), util/heap/layout/mmapper/csm/two_level_storage.rs (refactored to use `std::sync::OnceLock` instead of `OnceOptionBox`, removing `unsafe impl Send/Sync`), util/rust_util/atomic_box.rs (emptied unused file, removing 6 unsafe blocks), util/rust_util/zeroed_alloc.rs (emptied unused file, removing 2 unsafe blocks)
...
- "`InitializeOnce` is used for `SFT_MAP` to ensure zero-cost reads in hot paths. Do not replace with `OnceLock` for `SFT_MAP`."
...
## Patterns Discovered
- `Option<NonNull<T>>` with `unsafe impl Send/Sync` → `Arc<Mutex<T>>` (or `Arc<T>` if read-only) for shared state between parent and children without lifetimes. Eliminates raw pointer dereferences and unsafe Send/Sync.
...
- `*mut Atomic<T>` field in types that need to be `Send` → `Address` field. Convert to pointer only when needed in methods. This makes the type naturally `Send` and eliminates `unsafe impl Send`.
...
- `src/util/heap/blockpageresource.rs` — Completed (replaced `new_zeroed_vec` with uninitialized vec, removing dependency on `zeroed_alloc.rs`).
...
- `src/policy/immix/line.rs` — Completed (removed all unsafe blocks by using safe atomic load/store).
