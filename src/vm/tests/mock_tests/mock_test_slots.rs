...
mod compressed_oop {
    ...
    impl<'a> PartialEq for CompressedOopSlot<'a> {
        fn eq(&self, other: &Self) -> bool {
            std::ptr::eq(self.slot_addr, other.slot_addr)
        }
    }
    impl<'a> Eq for CompressedOopSlot<'a> {}
    impl<'a> std::hash::Hash for CompressedOopSlot<'a> {
        fn hash<H: std::hash::Hasher>(&self, state: &mut H) {
            (self.slot_addr as *const Atomic<u32>).hash(state)
        }
    }

    unsafe impl<'a> Send for CompressedOopSlot<'a> {}

    impl<'a> CompressedOopSlot<'a> {
        pub fn new(slot_addr: &'a Atomic<u32>) -> Self {
            Self { slot_addr }
        }
...
mod offset_slot {
    ...
    impl<'a> PartialEq for OffsetSlot<'a> {
        fn eq(&self, other: &Self) -> bool {
            std::ptr::eq(self.slot_addr, other.slot_addr) && self.offset == other.offset
        }
    }
    impl<'a> Eq for OffsetSlot<'a> {}
    impl<'a> std::hash::Hash for OffsetSlot<'a> {
        fn hash<H: std::hash::Hasher>(&self, state: &mut H) {
            (self.slot_addr as *const Atomic<Address>).hash(state);
            self.offset.hash(state);
        }
    }

    unsafe impl<'a> Send for OffsetSlot<'a> {}

    impl<'a> OffsetSlot<'a> {
        pub fn new_no_offset(slot_addr: &'a Atomic<Address>) -> Self {
            Self {
                slot_addr,
                offset: 0,
            }
        ...
        pub fn new_with_offset(slot_addr: &'a Atomic<Address>, offset: usize) -> Self {
            Self {
                slot_addr,
                offset,
            }
...
mod tagged_slot {
    ...
    impl<'a> PartialEq for TaggedSlot<'a> {
        fn eq(&self, other: &Self) -> bool {
            std::ptr::eq(self.slot_addr, other.slot_addr)
        }
    }
    impl<'a> Eq for TaggedSlot<'a> {}
    impl<'a> std::hash::Hash for TaggedSlot<'a> {
        fn hash<H: std::hash::Hasher>(&self, state: &mut H) {
            (self.slot_addr as *const Atomic<usize>).hash(state)
        }
    }

    unsafe impl<'a> Send for TaggedSlot<'a> {}
    ...
    impl<'a> TaggedSlot<'a> {
        ...
        pub fn new(slot_addr: &'a Atomic<usize>) -> Self {
            Self { slot_addr }
        }
    }
...
mod mixed {
    ...
    unsafe impl<'a> Send for DummyVMSlot<'a> {}
