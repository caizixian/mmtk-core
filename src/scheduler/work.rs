...
pub trait GCWorkContext: Send + 'static {
    ...
    /// Concrete plans and spaces may choose to move or not to move the
    /// objects the traced edges point to.
    type DefaultTracePolicy: TracePolicy<Self::VM>;

    /// The [`TracePolicy`] for pinning tracing. The traced objects must not be moved.
    ///
    /// -   For non-transitive pinning roots, `PinningTracePolicy` will be used to trace the edges
    ///     from roots to objects, but their descendants will be traced using `DefaultTracePolicy`.
    /// -   For transitive pinning roots, `PinningTracePolicy` will be used to trace the edges
    ...
    ///
    /// If a plan does not support object pinning, it should use `UnsupportedTracePolicy` for this
    /// type member.
