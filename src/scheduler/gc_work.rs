...
impl<C: GCWorkContext> GCWork<C::VM> for Prepare<C> {
    fn do_work(&mut self, worker: &mut GCWorker<C::VM>, mmtk: &'static MMTK<C::VM>) {
        ...
        let plan_dyn: &mut dyn Plan<VM = C::VM> = unsafe { mmtk.get_plan_mut() };
        let plan_mut: &mut C::PlanType = plan_dyn.downcast_mut::<C::PlanType>().unwrap();
...
impl<C: GCWorkContext> GCWork<C::VM> for ScanMutatorRoots<C> {
    fn do_work(&mut self, worker: &mut GCWorker<C::VM>, mmtk: &'static MMTK<C::VM>) {
        let mutator = self.0.take().unwrap();
