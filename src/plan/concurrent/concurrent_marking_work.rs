    ...
    ConcurrentTraceObjects<VM, P, KIND>
{
    ...
    fn flush(&mut self) {
        if !self.next_objects.is_empty() {
            ...
            let w = Self::new(objects, self.mmtk);
            self.mmtk.scheduler.work_buckets[WorkBucketStage::Concurrent].add(w);
        }
    }
    ...
    fn trace_object(&mut self, object: ObjectReference, worker: &mut GCWorker<VM>) -> ObjectReference {
        let new_object = self
            .plan
            .trace_object::<Self, KIND>(self, object, worker);
