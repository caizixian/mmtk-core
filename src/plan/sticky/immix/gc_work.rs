...
impl<VM: VMBinding> crate::scheduler::GCWorkContext for StickyImmixNurseryGCWorkContext<VM> {
    ...
    type DefaultProcessEdges =
        PolicyDrivenProcessEdges<VM, GenNurseryTracePolicy<VM, Self::PlanType, DEFAULT_TRACE>>;
    type PinningProcessEdges =
        PolicyDrivenProcessEdges<VM, GenNurseryTracePolicy<VM, Self::PlanType, TRACE_KIND_TRANSITIVE_PIN>>;
}
    ...
    for StickyImmixMatureGCWorkContext<VM, KIND>
{
    ...
    type PlanType = StickyImmix<VM>;
    type DefaultProcessEdges =
        PolicyDrivenProcessEdges<VM, PlanObjectTracePolicy<VM, Self::PlanType, KIND>>;
    type PinningProcessEdges =
        PolicyDrivenProcessEdges<VM, PlanObjectTracePolicy<VM, Self::PlanType, TRACE_KIND_TRANSITIVE_PIN>>;
}
