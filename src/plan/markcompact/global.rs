...
use super::gc_work::{
    CalculateForwardingAddress, Compact,
    UpdateReferences,
};
...
impl<VM: VMBinding> Plan for MarkCompact<VM> {
    ...
    fn schedule_collection(&'static self, scheduler: &GCWorkScheduler<VM>) {
        ...
        scheduler.work_buckets[WorkBucketStage::VMRefClosure]
            .set_sentinel(Box::new(VMProcessWeakRefs::<VM, MatureTracePolicy<VM, MarkCompact<VM>, {crate::policy::markcompactspace::TRACE_KIND_MARK}>>::new()));

        // VM-specific weak ref forwarding
        scheduler.work_buckets[WorkBucketStage::VMRefForwarding]
            .add(VMForwardWeakRefs::<VM, MatureTracePolicy<VM, MarkCompact<VM>, {crate::policy::markcompactspace::TRACE_KIND_FORWARD}>>::new());
