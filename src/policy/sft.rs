...
pub trait SFT {
    ...
    /// Trace objects through SFT. This provides an easy way for most plans to trace objects
    /// without the need to implement any plan-specific
...
impl SFT for EmptySpaceSFT {
    ...
    fn sft_trace_object(
        ...
        panic!(
            "Call trace_object() on {}, which maps to an empty space. The SFT trace does not support the fallback to vm_trace_object().",
            object,
        )
    }
}
