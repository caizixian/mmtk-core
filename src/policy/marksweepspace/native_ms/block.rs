...
impl Block {
    ...
    pub fn load_free_list(&self) -> Address {
        let addr = Block::FREE_LIST_TABLE.load_atomic::<usize>(self.start(), Ordering::SeqCst);
    ...
    pub fn load_local_free_list(&self) -> Address {
        let addr = Block::LOCAL_FREE_LIST_TABLE.load_atomic::<usize>(self.start(), Ordering::SeqCst);
        Address::ZERO.add(addr)
    }
    ...
    pub fn load_thread_free_list(&self) -> Address {
        let addr = Block::THREAD_FREE_LIST_TABLE.load_atomic::<usize>(self.start(), Ordering::SeqCst);
        Address::ZERO.add(addr)
    }
    ...
    pub fn load_prev_block(&self) -> Option<Block> {
        let prev = Block::PREV_BLOCK_TABLE.load_atomic::<usize>(self.start(), Ordering::SeqCst);
        NonZeroUsize::new(prev).map(Block)
    }

    pub fn load_next_block(&self) -> Option<Block> {
        let next = Block::NEXT_BLOCK_TABLE.load_atomic::<usize>(self.start(), Ordering::SeqCst);
    ...
    pub fn load_tls(&self) -> VMThread {
        ...
        VMThread(OpaquePointer::from_address(Address::ZERO.add(tls)))
    }
