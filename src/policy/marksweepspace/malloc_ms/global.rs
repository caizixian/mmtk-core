...
impl<VM: VMBinding> MallocSpace<VM> {
    ...
    fn sweep_chunk_mark_on_side(&self, chunk_start: Address, _mark_bit_spec: SideMetadataSpec) {
        // We disable bulk xor on mark bits by default because it results in inaccurate page accounting.
        // We sweep object one by one.
