...
impl ReferenceProcessors {
    ...
    pub fn forward_refs<VM: VMBinding>(&self, trace: &mut dyn ObjectTracer, mmtk: &'static MMTK<VM>) {
        debug_assert!(
        ...
        self.soft
            .forward::<VM>(trace, is_nursery_gc(mmtk.get_plan()));
        self.weak
            .forward::<VM>(trace, is_nursery_gc(mmtk.get_plan()));
        self.phantom
            .forward::<VM>(trace, is_nursery_gc(mmtk.get_plan()));
    ...
    pub fn retain_soft_refs<VM: VMBinding>(&self, trace: &mut dyn ObjectTracer, mmtk: &'static MMTK<VM>) {
...
impl ReferenceProcessor {
    ...
    fn keep_referent_alive(
        e: &mut dyn ObjectTracer,
    ...
    pub fn forward<VM: VMBinding>(&self, trace: &mut dyn ObjectTracer, _nursery: bool) {
        ...
        fn forward_reference<VM: VMBinding>(
        ...
        ) -> ObjectReference {
            {
                ...
                trace!(
                    ...
                    reference,
                    <VM as VMBinding>::VMObjectModel::get_current_size(reference)
                );
            }

            if let Some(old_referent) =
                <VM as VMBinding>::VMReferenceGlue::get_referent(reference)
            {
                let new_referent = ReferenceProcessor::trace_forward_object(trace, old_referent);
                <VM as VMBinding>::VMReferenceGlue::set_referent(reference, new_referent);
        ...
        sync.references = sync
            .references
            .iter()
            .map(|reff| forward_reference::<VM>(trace, *reff))
            .collect();
    ...
    fn retain<VM: VMBinding>(&self, trace: &mut dyn ObjectTracer, _nursery: bool) {
        ...
        for reference in sync.references.iter() {
            ...
            num_live += 1;
            if let Some(referent) = <VM as VMBinding>::VMReferenceGlue::get_referent(*reference)
            {
...
use crate::scheduler::gc_work::{GCTracerContext, TracePolicy};
...
pub(crate) struct SoftRefProcessing<VM: VMBinding, T: TracePolicy<VM>>(PhantomData<(VM, T)>);
impl<VM: VMBinding, T: TracePolicy<VM>> GCWork<VM> for SoftRefProcessing<VM, T> {
    fn do_work(&mut self, worker: &mut GCWorker<VM>, mmtk: &'static MMTK<VM>) {
        if !mmtk.state.is_emergency_collection() {
            ...
            // Retain soft references using the TracePolicy-based tracer.
            let tracer_ctx = GCTracerContext::<VM, T> {
                stage: WorkBucketStage::SoftRefClosure,
                _phantom: PhantomData,
            };
            tracer_ctx.with_tracer(worker, |tracer| {
...
impl<VM: VMBinding, T: TracePolicy<VM>> SoftRefProcessing<VM, T> {
    pub fn new() -> Self {
        Self(PhantomData)
    }
}
...
pub(crate) struct RefForwarding<VM: VMBinding, T: TracePolicy<VM>>(PhantomData<(VM, T)>);
impl<VM: VMBinding, T: TracePolicy<VM>> GCWork<VM> for RefForwarding<VM, T> {
    fn do_work(&mut self, worker: &mut GCWorker<VM>, mmtk: &'static MMTK<VM>) {
        let tracer_ctx = GCTracerContext::<VM, T> {
            stage: WorkBucketStage::RefForwarding,
            _phantom: PhantomData,
        };
        tracer_ctx.with_tracer(worker, |tracer| {
            mmtk.reference_processors.forward_refs::<VM>(tracer, mmtk);
        });
    }
}
impl<VM: VMBinding, T: TracePolicy<VM>> RefForwarding<VM, T> {
    pub fn new() -> Self {
        Self(PhantomData)
    }
}
