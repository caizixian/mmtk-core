...
impl<F: Finalizable> FinalizableProcessor<F> {
    ...
    fn forward_finalizable_reference<E: ObjectTraceProvider>(e: &mut E, finalizable: &mut F) {
        finalizable.keep_alive::<E>(e);
    }

    pub fn scan<E: ObjectTraceProvider>(&mut self, tls: VMWorkerThread, e: &mut E, nursery: bool) {
        ...
        <<E as ObjectTraceProvider>::VM as VMBinding>::VMCollection::schedule_finalization(tls);
    }

    pub fn forward_candidate<E: ObjectTraceProvider>(&mut self, e: &mut E, _nursery: bool) {
        self.candidates
            .iter_mut()
    ...
    pub fn forward_finalizable<E: ObjectTraceProvider>(&mut self, e: &mut E, _nursery: bool) {
...
#[derive(Default)]
pub struct Finalization<E: ObjectTraceProvider>(PhantomData<E>);

impl<E: ObjectTraceProvider> GCWork<E::VM> for Finalization<E> {
    fn do_work(&mut self, worker: &mut GCWorker<E::VM>, mmtk: &'static MMTK<E::VM>) {
        ...
        let mut w = E::new_provider(mmtk, WorkBucketStage::FinalRefClosure);
        w.set_worker(worker);
...
impl<E: ObjectTraceProvider> Finalization<E> {
    pub fn new() -> Self {
        Self(PhantomData)
    }
}
...
#[derive(Default)]
pub struct ForwardFinalization<E: ObjectTraceProvider>(PhantomData<E>);

impl<E: ObjectTraceProvider> GCWork<E::VM> for ForwardFinalization<E> {
    fn do_work(&mut self, worker: &mut GCWorker<E::VM>, mmtk: &'static MMTK<E::VM>) {
        trace!("Forward finalization");
        let mut finalizable_processor = mmtk.finalizable_processor.lock().unwrap();
        let mut w = E::new_provider(mmtk, WorkBucketStage::FinalizableForwarding);
        w.set_worker(worker);
...
impl<E: ObjectTraceProvider> ForwardFinalization<E> {
    pub fn new() -> Self {
        Self(PhantomData)
    }
}
