...
use crate::vm::{Collection, ObjectTracer, ObjectTracerContext, VMBinding};
use crate::MMTK;
...
impl<F: Finalizable> FinalizableProcessor<F> {
    ...
    fn forward_finalizable_reference(e: &mut dyn ObjectTracer, finalizable: &mut F) {
        finalizable.keep_alive(e);
    }

    pub fn scan<VM: VMBinding>(&mut self, tls: VMWorkerThread, e: &mut dyn ObjectTracer, nursery: bool) {
        ...
        <VM as VMBinding>::VMCollection::schedule_finalization(tls);
    }

    pub fn forward_candidate(&mut self, e: &mut dyn ObjectTracer, _nursery: bool) {
        self.candidates
            .iter_mut()
            .for_each(|f| FinalizableProcessor::<F>::forward_finalizable_reference(e, f));
    }

    pub fn forward_finalizable(&mut self, e: &mut dyn ObjectTracer, _nursery: bool) {
...
impl<VM: VMBinding, T: TracePolicy<VM>> GCWork<VM> for Finalization<VM, T> {
    fn do_work(&mut self, worker: &mut GCWorker<VM>, mmtk: &'static MMTK<VM>) {
        ...
        tracer_ctx.with_tracer(worker, |tracer| {
            finalizable_processor.scan::<VM>(tls, tracer, is_nursery_gc(mmtk.get_plan()));
        });
