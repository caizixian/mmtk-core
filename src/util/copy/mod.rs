...
impl<VM: VMBinding> GCWorkerCopyContext<VM> {
    ...
    pub fn new(worker_tls: VMWorkerThread, mmtk: &MMTK<VM>, config: CopyConfig<VM>) -> Self {
        let mut ret = GCWorkerCopyContext {
            copy: [const { MaybeUninit::uninit() }; MAX_COPYSPACE_COPY_ALLOCATORS],
            immix: [const { MaybeUninit::uninit() }; MAX_IMMIX_COPY_ALLOCATORS],
            immix_hybrid: [const { MaybeUninit::uninit() }; MAX_IMMIX_HYBRID_COPY_ALLOCATORS],
            config,
        };
