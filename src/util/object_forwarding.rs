...
pub fn read_forwarding_pointer<VM: VMBinding>(object: ObjectReference) -> ObjectReference {
    ...
    let addr_usize = VM::VMObjectModel::LOCAL_FORWARDING_POINTER_SPEC.load_atomic::<VM, usize>(
        object,
        Some(FORWARDING_POINTER_MASK),
        Ordering::SeqCst,
    );
    let addr = unsafe { crate::util::Address::from_usize(addr_usize) };
    ObjectReference::from_raw_address(addr).expect("Invalid forwarding pointer")
}
