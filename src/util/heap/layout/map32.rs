...
pub struct Map32 {
    ...
    next_link: Vec<AtomicI32>,
    prev_link: Vec<AtomicI32>,
...
impl Map32 {
    pub fn new() -> Self {
        ...
        Map32 {
            descriptor_map: (0..max_chunks).map(|_| AtomicUsize::new(SpaceDescriptor::UNINITIALIZED.as_usize())).collect(),
            next_link: (0..max_chunks).map(|_| AtomicI32::new(0)).collect(),
            prev_link: (0..max_chunks).map(|_| AtomicI32::new(0)).collect(),
...
impl VMMap for Map32 {
    fn insert(&self, start: Address, extent: usize, descriptor: SpaceDescriptor) {
        ...
        while e < extent {
            ...
            let old = self.descriptor_map[index].load(Ordering::Relaxed);
            assert!(
                old == SpaceDescriptor::UNINITIALIZED.as_usize(),
    ...
    unsafe fn allocate_contiguous_chunks(
        ...
        if head.is_zero() {
            debug_assert!(self.next_link[chunk as usize].load(Ordering::Relaxed) == 0);
        } else {
            self.next_link[chunk as usize].store(head.chunk_index() as _, Ordering::Relaxed);
            self.prev_link[head.chunk_index()].store(chunk, Ordering::Relaxed);
        }
        debug_assert!(self.prev_link[chunk as usize].load(Ordering::Relaxed) == 0);
        rtn
    }
    ...
    fn get_next_contiguous_region(&self, start: Address) -> Address {
        ...
        if chunk == 0 || next == 0 {
            ...
            conversions::chunk_index_to_address(next as _)
        }
    }
    ...
    fn get_available_discontiguous_chunks(&self) -> usize {
        let inner = self.inner.lock().unwrap();
        inner.total_available_discontiguous_chunks
    }
    ...
    fn get_descriptor_for_address(&self, address: Address) -> SpaceDescriptor {
        ...
        self.descriptor_map
            .get(index)
            .map(|atomic| SpaceDescriptor::from_usize(atomic.load(Ordering::Relaxed)))
...
impl Map32 {
    fn free_contiguous_chunks_no_lock(&self, inner: &mut Map32Inner, chunk: i32) -> usize {
        ...
        let next = self.next_link[chunk as usize].load(Ordering::Relaxed);
        let prev = self.prev_link[chunk as usize].load(Ordering::Relaxed);
        if next != 0 {
            self.prev_link[next as usize].store(prev, Ordering::Relaxed);
        }
        if prev != 0 {
            self.next_link[prev as usize].store(next, Ordering::Relaxed);
        }
        self.prev_link[chunk as usize].store(0, Ordering::Relaxed);
        self.next_link[chunk as usize].store(0, Ordering::Relaxed);
