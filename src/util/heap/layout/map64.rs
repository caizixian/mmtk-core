...
impl VMMap for Map64 {
    ...
    fn create_parent_freelist(
        ...
        inner.high_water[index] = base;
        inner.base_address[index] = base;
    ...
    unsafe fn allocate_contiguous_chunks(
        ...
        let rtn = inner.high_water[index];
        let extent = chunks << LOG_BYTES_IN_CHUNK;
        inner.high_water[index] = rtn + extent;
        ...
        if let Some(freelist) = maybe_freelist {
            ...
            let base_page = conversions::bytes_to_pages_up(rtn - inner.base_address[index]);
    ...
    fn get_descriptor_for_address(&self, address: Address) -> SpaceDescriptor {
        if let Some(index) = Self::space_index(address) {
            self.inner.lock().descriptor_map[index]
        } else {
