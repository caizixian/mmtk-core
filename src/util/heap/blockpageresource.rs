...
struct BlockQueue<B: Region> {
    ...
    /// -   `Option<T>`: It may contain uninitialized elements as `None`.
    ///
    /// The implementation of `BlockQueue` must ensure there is no data race, and it never reads
...
impl<B: Region> BlockQueue<B> {
    ...
    fn get_entry(&self, i: usize) -> B {
        unsafe { (*self.data.get())[i].expect("Entry should be initialized") }
    }
