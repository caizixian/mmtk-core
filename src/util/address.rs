...
impl ObjectReference {
    ...
    pub fn is_reachable(self) -> bool {
        SFT_MAP.get_checked(self.to_raw_address()).is_reachable(self)
    }

    /// Is the object live, determined by the policy?
    pub fn is_live(self) -> bool {
        SFT_MAP.get_checked(self.to_raw_address()).is_live(self)
    }

    /// Can the object be moved?
    pub fn is_movable(self) -> bool {
        SFT_MAP.get_checked(self.to_raw_address()).is_movable()
    }

    /// Get forwarding pointer if the object is forwarded.
    pub fn get_forwarded_object(self) -> Option<Self> {
        SFT_MAP.get_checked(self.to_raw_address()).get_forwarded_object(self)
    }

    /// Is the object in any MMTk spaces?
    pub fn is_in_any_space(self) -> bool {
        SFT_MAP.get_checked(self.to_raw_address()).is_in_space(self)
    }

    /// Is the object sane?
    #[cfg(feature = "sanity")]
    pub fn is_sane(self) -> bool {
        SFT_MAP.get_checked(self.to_raw_address()).is_sane()
    }
}
