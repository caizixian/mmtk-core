...
impl<VM: VMBinding> ImmixAllocator<VM> {
    ...
    fn set_limit_for_stress(&mut self) {
        if self.bump_pointer.cursor < self.bump_pointer.limit {
            ...
            let new_limit = Address::ZERO.add(self.bump_pointer.limit - self.bump_pointer.cursor);
        ...
        if self.large_bump_pointer.cursor < self.large_bump_pointer.limit {
            ...
            let new_lg_limit = Address::ZERO.add(self.large_bump_pointer.limit - self.large_bump_pointer.cursor);
