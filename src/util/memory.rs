...
/// Zero a region of memory using non-temporal (streaming) stores.
///
/// Non-temporal stores bypass the CPU cache hierarchy, writing directly to main memory.
/// This is beneficial for zeroing large memory regions (e.g., newly acquired pages) that
/// won't be accessed immediately, as it avoids evicting useful data from the cache.
///
/// Based on the analysis in Blackburn et al., "Fast Conservative Garbage Collection",
/// OOPSLA 2011, non-temporal zeroing nearly doubles effective zeroing bandwidth on modern
/// CPUs and reduces cache pollution, leading to measurable overall performance improvements.
///
/// On non-x86_64 architectures, this falls back to regular `zero()` (memset).
pub fn nontemporal_zero(start: Address, len: usize) {
    #[cfg(target_arch = "x86_64")]
    {
        nontemporal_zero_x86(start, len);
    }
...
/// x86_64 implementation of non-temporal zeroing using SSE2 streaming stores.
#[cfg(target_arch = "x86_64")]
fn nontemporal_zero_x86(start: Address, len: usize) {
    use std::arch::x86_64::{__m128i, _mm_setzero_si128, _mm_sfence, _mm_stream_si128};

    // For small regions, the setup overhead of non-temporal stores isn't worthwhile.
    // The threshold is chosen based on the paper's observation that NT stores have
    // higher per-instruction cost but better throughput for large regions.
    const NT_THRESHOLD: usize = 256;
    ...
    unsafe {
        let mut ptr = start.to_mut_ptr::<u8>();
        let end = ptr.add(len);
        let zero_vec: __m128i = _mm_setzero_si128();

        // Handle unaligned prefix: zero bytes until we reach 16-byte alignment
        let align_offset = ptr.align_offset(16);
        if align_offset > 0 {
            let prefix_len = align_offset.min(len);
            std::ptr::write_bytes(ptr, 0, prefix_len);
            ptr = ptr.add(prefix_len);
        }

        // Main loop: 128-byte unrolled non-temporal stores (8x 16-byte stores)
        let aligned_end = end.sub(end as usize % 16);
        while ptr.add(128) <= aligned_end {
            _mm_stream_si128(ptr.cast::<__m128i>(), zero_vec);
            _mm_stream_si128(ptr.add(16).cast::<__m128i>(), zero_vec);
            _mm_stream_si128(ptr.add(32).cast::<__m128i>(), zero_vec);
            _mm_stream_si128(ptr.add(48).cast::<__m128i>(), zero_vec);
            _mm_stream_si128(ptr.add(64).cast::<__m128i>(), zero_vec);
            _mm_stream_si128(ptr.add(80).cast::<__m128i>(), zero_vec);
            _mm_stream_si128(ptr.add(96).cast::<__m128i>(), zero_vec);
            _mm_stream_si128(ptr.add(112).cast::<__m128i>(), zero_vec);
            ptr = ptr.add(128);
        }

        // Handle remaining 16-byte aligned chunks
        while ptr.add(16) <= aligned_end {
            _mm_stream_si128(ptr.cast::<__m128i>(), zero_vec);
            ptr = ptr.add(16);
        }

        // Handle trailing bytes (< 16 bytes)
        if ptr < end {
            std::ptr::write_bytes(ptr, 0, end as usize - ptr as usize);
        }

        // Ensure all non-temporal stores are globally visible before any subsequent
        // stores. This is necessary because NT stores are weakly ordered.
        _mm_sfence();
    }
}
