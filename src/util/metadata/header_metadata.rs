...
mod tests {
        ...
        ($type: ty) => {
            paste!{
                fn [<with_ $type _obj>]<F>(f: F) where F: FnOnce(Address, &mut [$type]) + std::panic::UnwindSafe {
                    ...
                    let obj = Address::from_ref(&storage[1]);
                    f(obj, &mut storage);
                }
            }
        }
    ...
    fn test_load<T: MetadataValue>(spec: HeaderMetadataSpec, obj: Address, mask: Option<T>) -> T {
        unsafe { spec.load(obj, mask) }
    }

    fn test_store<T: MetadataValue>(spec: HeaderMetadataSpec, obj: Address, val: T, mask: Option<T>) {
        unsafe { spec.store(obj, val, mask) }
    }
            ...
            paste!{
                ...
                fn [<$tname _load>]() {
                    [<with_ $type _obj>](|obj, ptr| {
                        ...
                        assert_eq!(test_load::<$type>(spec, obj, None), 0);
                        let max_value = max_value($num_of_bits) as $type;
                        ptr[1] = max_value;
                        assert_eq!(test_load::<$type>(spec, obj, None), max_value);
                    });
                }
                ...
                fn [<$tname _load_mask>]() {
                    [<with_ $type _obj>](|obj, ptr| {
                        ...
                        ptr[1] = max_value;
                        assert_eq!(test_load::<$type>(spec, obj, None), max_value);
                        assert_eq!(test_load::<$type>(spec, obj, Some(0)), 0);
                        assert_eq!(test_load::<$type>(spec, obj, Some(0b101)), 0b101);
                    });
                }
                ...
                fn [<$tname _store>]() {
                    [<with_ $type _obj>](|obj, ptr| {
                        ...
                        assert_eq!(test_load::<$type>(spec, obj, None), 0);
                        let max_value = max_value($num_of_bits) as $type;
                        test_store::<$type>(spec, obj, max_value, None);
                        assert_eq!(test_load::<$type>(spec, obj, None), max_value);
                ...
                fn [<$tname _store_mask>]() {
                    [<with_ $type _obj>](|obj, ptr| {
                        ...
                        test_store::<$type>(spec, obj, max_value, Some(max_value));
                        assert_eq!(test_load::<$type>(spec, obj, None), max_value);

                        test_store::<$type>(spec, obj, 0, None);

                        test_store::<$type>(spec, obj, max_value, Some(0b10));
                        assert_eq!(test_load::<$type>(spec, obj, None), 0b10);
                ...
                fn [<$tname _compare_exchange_success>]() {
                    [<with_ $type _obj>](|obj, _| {
                        let spec = HeaderMetadataSpec { bit_offset: 0, num_of_bits: $num_of_bits };
                        let old_val = test_load::<$type>(spec, obj, None);
                        assert_eq!(old_val, 0);
                ...
                fn [<$tname _compare_exchange_fail>]() {
                    [<with_ $type _obj>](|obj, _| {
                        ...
                        test_store::<$type>(spec, obj, 1, None);

                        let max_value = max_value($num_of_bits) as $type;
                        let res = spec.compare_exchange::<$type>(obj, old_val, max_value, None, Ordering::SeqCst, Ordering::SeqCst);
                        assert!(res.is_err());
                        assert_eq!(res.err().unwrap(), 1);
                        assert_eq!(test_load::<$type>(spec, obj, None), 1);
                    })
                }
