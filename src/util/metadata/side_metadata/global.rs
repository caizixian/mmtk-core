...
impl SideMetadataSpec {
    ...
    pub const fn get_absolute_offset(&self) -> Address {
        ...
        match self.offset {
            ...
            _ => panic!("Not an absolute offset"),
        }
    }
    ...
    pub const fn get_rel_offset(&self) -> usize {
        ...
        match self.offset {
            SideMetadataOffset::Relative(rel_offset) => rel_offset,
            _ => panic!("Not a relative offset"),
        }
    }
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.write_fmt(format_args!(
            ...
            **log_bytes_in_region: 0x{:x} \
            ...
            self.is_global,
            match self.offset {
                SideMetadataOffset::Absolute(addr) => format!("Absolute(0x{:x})", addr),
                SideMetadataOffset::Relative(rel_offset) => format!("Relative(0x{:x})", rel_offset),
            },
            self.log_num_of_bits,
            self.log_bytes_in_region
        ))
